...
func TestBasicParse(t *testing.T) {
	...
	testCases := []struct {
		...
		{`null`, `null`, PARSE_OK},
		{`true`, `true`, PARSE_OK},
		{`false`, `false`, PARSE_OK},
		{`123.456`, `123.456`, PARSE_OK},
		{`"Hello, World!"`, `"Hello, World!"`, PARSE_OK},
		{`[1, true, null, "text"]`, `[1,true,null,"text"]`, PARSE_OK},
		{`{"name": "John", "age": 30}`, `{"name":"John","age":30}`, PARSE_OK},
		{``, ``, PARSE_EXPECT_VALUE},
		{`  `, ``, PARSE_EXPECT_VALUE},
		{`invalid`, ``, PARSE_INVALID_VALUE},
		{`null x`, ``, PARSE_ROOT_NOT_SINGULAR},
	}

	for _, tc := range testCases {
		t.Run(fmt.Sprintf("Parse(%s)", tc.json), func(t *testing.T) {
			var v Value
			err := Parse(&v, tc.json)
			if err != tc.err {
				t.Errorf("期望错误 %v，得到 %v", tc.err, err)
			}
			if err == PARSE_OK {
				result := v.String()
...
func TestEnhancedErrors(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			expectedLine:   1,
			expectedColumn: 6,
			containsText:   "缺少逗号或方括号",
		},
		{
			name:           "对象缺少键",
			json:           `{1:1}`,
			expectedCode:   PARSE_MISS_KEY,
			expectedLine:   1,
			expectedColumn: 2,
			containsText:   "缺少键",
		},
		...
		},
		{
			name:           "对象缺少闭合",
			json:           `{"a": 1,`,
			expectedCode:   PARSE_MISS_COMMA_OR_CURLY_BRACKET,
			expectedLine:   1,
			expectedColumn: 9,
			containsText:   "缺少逗号或花括号",
		},
		{
			name:           "多行JSON错误",
			json:           "{\n\"a\": 1,\n\"b\": \ntrue,\n\"c\": [1, 2,\n]\n",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// 使用带选项的解析
			var actualError *EnhancedError
			err := ParseWithOptions(&v, tc.json, options)

			if err != tc.expectedCode {
				t.Errorf("期望错误代码 %v，得到 %v", tc.expectedCode, err)
			}

			// 从上下文中获取增强错误信息
			// 注意：在实际实现中，需要改变Parse的返回值类型为*EnhancedError
			// 或提供一个函数获取最后一次解析的错误详情
			// 这里我们使用一个模拟的方式来测试
			ctx := newContext(tc.json, options)
			ctx.index = len(tc.json) // 模拟解析到结束位置
			actualError = ctx.createError(err, "")

			// 检查错误详情
			if actualError.Line != tc.expectedLine || actualError.Column != tc.expectedColumn {
				t.Errorf("期望位置 [%d:%d]，得到 [%d:%d]",
					tc.expectedLine, tc.expectedColumn,
					actualError.Line, actualError.Column)
			}

			// 检查错误消息
			errorMsg := actualError.Error()
			if !strings.Contains(errorMsg, tc.containsText) {
				t.Errorf("期望错误消息包含 %q，得到 %q", tc.containsText, errorMsg)
			}
		})
	}
}
...
func TestComments(t *testing.T) {
	testCases := []struct {
		...
		{
			name:     "未闭合的注释",
			json:     `/* unclosed comment`,
			expected: ``,
			err:      PARSE_COMMENT_NOT_CLOSED,
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			// 将 \n 替换为实际的换行符
			jsonStr := strings.Replace(tc.json, `\n`, "\n", -1)
			...
			err := ParseWithOptions(&v, jsonStr, options)
			if err != tc.err {
				t.Errorf("期望错误 %v，得到 %v", tc.err, err)
			}
...
func TestMaxDepth(t *testing.T) {
	// 创建一个嵌套深度为5的数组
	const depth = 5
	json := strings.Repeat("[", depth) + "1" + strings.Repeat("]", depth)
	...
	t.Run("超过最大深度", func(t *testing.T) {
		...
		options := ParseOptions{
			RecoverFromErrors: false,
			AllowComments:     false,
			MaxDepth:          depth - 1, // 设置比实际深度小的限制
		}

		err := ParseWithOptions(&v, json, options)
		if err != PARSE_MAX_DEPTH_EXCEEDED {
			t.Errorf("期望错误 PARSE_MAX_DEPTH_EXCEEDED，得到 %v", err)
		}
	})

	// 设置的最大深度大于等于实际深度，应该成功
	t.Run("不超过最大深度", func(t *testing.T) {
		var v Value
		options := ParseOptions{
			RecoverFromErrors: false,
			AllowComments:     false,
			MaxDepth:          depth, // 设置等于实际深度的限制
		}

		err := ParseWithOptions(&v, json, options)
		if err != PARSE_OK {
			t.Errorf("期望错误 PARSE_OK，得到 %v", err)
		}
	})
}
